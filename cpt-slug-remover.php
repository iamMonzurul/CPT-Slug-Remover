<?php
/*
Plugin Name: CPT Slug Remover
Plugin URI: http://monzurulhaque.com/cpt-slug-remover/
Description: Removes the slug from permalink on selected custom post type based on <a href="https://wordpress.org/plugins/remove-slug-from-custom-post-type/">Remove slug from custom post type</a> by Joakim Ling.
Version: 1.0.0
Author: Monzurul Haque
Author URI: http://monzurulhaque.com/
*/
...
class UWT_RemoveSlugCustomPostType{
	...
	function cancel_redirect_canonical($redirect_url) {
	    $post_type_name = get_option('uwt_permalink_customtype_name');
		...
		if (post_type_exists($post_type_name)) {
            $custom_post			= $wp_post_types[$post_type_name];
	...
	public function menu() {
		add_options_page( 'CPT Slug Remover from Selected Custom Post Type', 'CPT Slug Remover', 'manage_options', __FILE__, array(&$this, 'menu_page'));
	}
	...
	public function menu_page() {
		...
		$opt_name = 'uwt_permalink_customtype_name';
		$opt_name_val = get_option($opt_name);

		$opt_suffix = 'uwt_permalink_customtype_suffix';
		$opt_suffix_val = get_option($opt_suffix);

		if(isset($_POST[$hidden_field_name]) && $_POST[$hidden_field_name] == 'Y') {
			$opt_name_val = preg_replace('/[^0-9a-z]+/i', '', $_POST[$opt_name]);
			update_option($opt_name, $opt_name_val);

			$opt_suffix_val = preg_replace('/[^0-9a-z]+/i', '', $_POST[$opt_suffix]);
			update_option($opt_suffix, $opt_suffix_val);
?>
...
<?php
		...
		echo '<div class="wrap">';
		echo "<h2>CPT Slug Remover</h2>";
?>
	<form name="form1" method="post" action="">
		<input type="hidden" name="<?php echo $hidden_field_name; ?>" value="Y">
		<p>Custom Post Type Name/Slug:
			<input type="text" name="<?php echo $opt_name; ?>" value="<?php echo $opt_name_val; ?>" size="20">
			<br />
			<em>Please put the custom post type name or slug here.</em>
		</p>
		<hr />
		<p>Permalink custom type suffix:
			<input type="text" name="<?php echo $opt_suffix; ?>" value="<?php echo $opt_suffix_val; ?>" size="20">
			<br />
...
<?php
	...
	public function post_save($post_id) {
		...
		if (post_type_exists($post_type_name)) {
			...
			if ($custom_post->_builtin == false && $custom_post->name == $post_type) {
				$this->rewrite_rules(true);
				$this->add_rules_htaccess();				
			}
		}
	...
	public function remove_slug($permalink, $post, $leavename) {
		...
		if (post_type_exists($post_type_name)) {
            ...
            if (!empty($post_type_name) && $custom_post->name == $post_type_name && $custom_post->_builtin == false) {
			...
			if ($custom_post->_builtin == false) {
				$querystr = "SELECT {$wpdb->posts}.post_name
								...
								WHERE {$wpdb->posts}.post_status = 'publish'
										AND {$wpdb->posts}.post_type = '{$post_type_name}'
